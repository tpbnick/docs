...
We also have the modulus operator (`#!c %`), which gives us the remainder when
...
To see a list of files in a directory – type `ls` Files with \* means it is
...
We will rename `#!c int i` from earlier to `#!c int rows` and create
