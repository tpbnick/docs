...
![!Running Time](https://cdn.nickplatt.dev/Docs/running_time.png)
...
The next pair, `6` and `8`, are in order, so we don’t need to swap them. The
next pair, `8` and `5`, need to be swapped:
...
- Since we are comparing the `i'th` and `i+1'th` element, we only need to go up
  to _n_ – 2 for `i`. Then, we swap the two elements if they’re out of order.
...
- Now, our `draw` function first calls itself **recursively**, drawing a pyramid
  of height `h - 1`. But even before that, we need to stop if `h` is 0, since
  there won’t be anything left to drawn.
