...
- Insertion is complex - a lot of dynamic memory allocation, but gets easier as
  you go.
...
One solution might be to allocate more memory in a larger area that's free, and
...
**Data structures** are programming constructs that allow us to store
...
different types, typically. Arrays are great for element lookup, but unless we
want to insert at the very end of an array, inserting elements is quite costly.
Arrays also suffer from great inflexibility - what happens if we need a larger
array than we thought?
...
![!linked-list](https://cdn.nickplatt.dev/Docs/linked_list.png)

- This is different than an array since our values are no longer next to one
  another in memory.

![linked-list-with-addresses](https://cdn.nickplatt.dev/Docs/linked_list_with_addresses.png)

- This uses two chunks of memory, where the second chunk is used to point at the
  next chunk of memory. By the way, `NUL` refers to `\0`, a character that ends
  a string, and `NULL` refers to an address of all zeros, or a null pointer that
  we can think of as pointing nowhere. These chunks are linked by the pointers
  in the second chunk of memory.
...
- If not, set the traversal point to the next pointer in the list and go back to
  step 2 (above).
...
![!list-with-one-node](https://cdn.nickplatt.dev/Docs/list_with_one_node.png)
...
- First, a **hash function**, which returns a nonnegative integer value called a
  _hash code_.

- Second, an **array** capable of storing data of the type we wish to place into
  the data structure.
...
![!hash-table](https://cdn.nickplatt.dev/Docs/hash_table.png)
...
![!trie](https://cdn.nickplatt.dev/Docs/trie.png)
...
![!binary-search-tree](https://cdn.nickplatt.dev/Docs/binary_search_tree.png)
