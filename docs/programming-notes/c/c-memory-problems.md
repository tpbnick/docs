...
![!bitmap](https://cdn.nickplatt.dev/Docs/bitmap.png)
...
![!red-smile](https://cdn.nickplatt.dev/Docs/red_smile.png)
...
![!grid](https://cdn.nickplatt.dev/Docs/grid.png)
...
Finally, let’s look at `Makefile`. This file specifies what should happen when
...
- The function `sepia` should take an image and turn it into a sepia version of
  the same image.
...
Fortunately, digital cameras tend to store photographs contiguously on memory
...
JPEGs you recover; they should still be viewable. Now, I only have one memory
card, but there are a lot of you! And so I’ve gone ahead and created a “forensic
image” of the card, storing its contents, byte after byte, in a file called
`card.raw`. So that you don’t waste time iterating over millions of 0s
unnecessarily, I’ve only imaged the first few megabytes of the memory card. But
...
But having to write out all 16 variations could become tedious. For a shortcut,
